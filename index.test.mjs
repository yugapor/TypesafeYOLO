...
test("the tool gate allows, asks, denies, and stays closed on failure or cancellation", async (t) => {
  ...
  writeFileSync(join(agentDir, "typesafe-yolo.key"), "test-key\n");
  t.after(() => {
  ...
  const fetchMock = t.mock.method(globalThis, "fetch", async (url, options) => {
    requests.push({ url, auth: options.headers.Authorization, body: JSON.parse(options.body), expectedFilter });
  ...
  rmSync(join(agentDir, "typesafe-yolo.key"));
  handler = await load();
  ctx.signal = new AbortController().signal;
  approval = "Deny";
  let message;
  ctx.ui.select = async (m) => { message = m; return approval; };
  assert.equal((await handler(event, ctx)).block, true);
  assert.match(message, /typesafe-yolo\.key/);
  assert.equal(fetchMock.mock.callCount(), calls);
  ...
  for (const { url, auth, body, expectedFilter } of requests) {
    assert.equal(url, "https://api.typesafe.ai/v1/systemone");
    assert.equal(auth, "Bearer test-key");
    assert.equal(body.state.api_key_file, join(agentDir, "typesafe-yolo.key"));
